...
use FindBin;
use lib "$FindBin::Bin/../../../wurst/blib/arch";
use lib "$FindBin::Bin/../../../wurst/blib/lib";
...
use vars qw ( $INPUT_CLST_LIST $OUTPUT_BIN_DIR $PDB_TOP_DIR
              $OUTPUT_LIB_LIST);
$INPUT_CLST_LIST= '/smallfiles/public/no_backup/pdb/derived_data/NR/clusters90.txt';
$PDB_TOP_DIR    = '/smallfiles/public/no_backup/pdb/data/structures/divided/pdb';
$OUTPUT_BIN_DIR = "/smallfiles/public/no_backup/bm/pdb_all_bin";
$OUTPUT_LIB_LIST= '/smallfiles/public/no_backup/bm/pdb_all.list';
...
sub mymain ()
{
    ...
    my $r = read_cluster_file($INPUT_CLST_LIST, $first_clust, $last_clust,
...
#   cls_dumper (\@clust_acq, \@clust_chain);
    ...
    if (final_lib_list ( $OUTPUT_LIB_LIST, $f_sfx, \@lib_list)== EXIT_FAILURE){
        return (EXIT_FAILURE); }
