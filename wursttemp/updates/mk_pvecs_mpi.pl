...
use lib "/smallfiles/public/bm/salamiServer";
use Salamisrvini;

use lib "$LIB_ARCH/auto/Wurst";
use lib $LIB_LIB;

use Wurst;

#@TODO move that crap from Thommi's dir into ... salamiServer in cluster
use lib "$MPI_LIB_BASE/blib/arch/auto/Parallel/MPI/Simple";
use lib "$MPI_LIB_BASE/blib/lib";
...
#use vars qw ($PVEC_STRCT_DIR); use from Salamisrv.ini
#$PVEC_STRCT_DIR       = '/smallfiles/public/no_backup/bm/pdb_all_vec_6mer_struct';

use vars qw( @PROFILE_DIRS  $phd_suffix $prof_suffix);
...
#*bin_suffix       = \'.bin'; from Salamisrvini
		...
		if( -e "$PVEC_STRCT_DIR/$msg.vec" ){    
            ...
            unless (-e $struct_path){
	         ...
	         my $pvec = strct_2_prob_vec($struct, $classfcn, 1);
                 if ($pvec) {
                     prob_vec_write($pvec, "$PVEC_STRCT_DIR/$msg.vec");
                 } else {
                   print "failed to calculate vector 6 for $msg\n";
                 }
            }
        }
        if( -e "$PVEC_CA_DIR/$msg.vec" ){    
            ...
            unless (-e $struct_path){
                ...
                if ($pvec) {
                  ...
                  print "failed to calcualte vector 7 for $msg\n";
                }
            }
        }
		$msg = MPI_Recv(0, 123, MPI_COMM_WORLD);
