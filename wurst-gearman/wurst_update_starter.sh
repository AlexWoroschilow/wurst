...
SCRIPT_CLIENT="$(pwd)/gearman/planner.pl"
...
FILE_SETTINGS_SRC="$(pwd)/input";
FILE_SETTINGS_SERVER="${FILE_SETTINGS_SRC}/server.conf";


FILE_PDB_LIB="$(pwd)/pdb_lib.list";
FILE_PDB_ALL="$(pwd)/pdb_all.list";
FILE_PDB_SLM="$(pwd)/pdb_slm.list";
FILE_PDB_90N="$(pwd)/pdb_90n.list";
...
echo "Run: ${SCRIPT_CLIENT}";
${SCRIPT_CLIENT} 1>>${FILE_STD_OUT} 2>> ${FILE_STD_ERR} &
CLIENT_PID=$!

sleep 3;

echo "Run: ${SCRIPT_WORKER}";
${SCRIPT_WORKER} 1>>${FILE_STD_OUT} 2>> ${FILE_STD_ERR} &
echo "Run: ${SCRIPT_WORKER}";
${SCRIPT_WORKER} 1>>${FILE_STD_OUT} 2>> ${FILE_STD_ERR} &
echo "Run: ${SCRIPT_WORKER}";
${SCRIPT_WORKER} 1>>${FILE_STD_OUT} 2>> ${FILE_STD_ERR} &
